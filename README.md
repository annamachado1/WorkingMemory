...
- **Hit rate** is `n_hits / n_chases`, not `n_hits / n_trials`. A trial in which
  no chase ever started (ended by a false alarm) contributes no chase to detect.
- **False-alarm rate** should use `time_at_risk_s` as the denominator, since
  participants are free to press at any moment during a trial.
- **Unresolved chases** are censored, not misses. Decide explicitly whether to
  drop them or treat them as right-censored observations.
- **Discarded presses** are excluded from both hits and false alarms. A high
  `n_discarded` suggests the participant is pressing rhythmically rather than
  responding to the stimulus.
- **`rt_ms`** is taken from the keyboard's hardware timestamp where that is
  available and plausible, falling back to the frame clock otherwise.
- **Degrees of visual angle** can be recovered from the sidecar: it records
  screen width in cm, viewing distance in cm, and the window's pixel dimensions.
- **The two onset distances differ.** `onset_dist` is how far apart the chaser
  and prey were when pursuit began — the geometry the participant has to work
  with. `onset_dist_from_trial_start` is how long that predator had been drifting
  around before it was selected, which indexes how "settled" the display was.
  A value near zero means the chase began almost immediately after the reset.

### Two things to watch

**Old data files will not match.** Headers are written only when a file does not
already exist, so appending to a CSV from an earlier version of the script will
silently misalign columns. Move or rename anything in `data/` after changing the
event or block schema.

**Reproducibility is partial.** The seed is the subject number, so the *first*
dot placement of a session is reproducible. Beyond that, every frame and every
trial consumes random draws, and the number of frames per trial depends on when
the participant responded — so two runs of the same subject number diverge as
soon as the responses differ. The seed makes a run auditable, not replayable.

### Known simplifications

- `chaser_group` records which of two selection pools a chaser was drawn from,
  but both pools are drawn in the same black, so the split is invisible to the
  participant and carries no perceptual meaning.
- A trial has no time limit. If the participant never responds, it runs until the
  chaser catches the prey, which can take upwards of ten seconds.
